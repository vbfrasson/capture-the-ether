...
def fund_with_link(contract_address, account=None, link_token=None, amount=10 ** 28):
...
def skip_unit_test_if_network_not_local():
    if network.show_active() not in LOCAL_BLOCKCHAIN_ENVIRONMENTS:
        pytest.skip("Unit tests are for local testing only")


def skip_integration_test_if_network_local():
    if network.show_active() in LOCAL_BLOCKCHAIN_ENVIRONMENTS:
